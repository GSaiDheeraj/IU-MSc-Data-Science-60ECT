...
def get_stock_data(company_name, num_months):
    ...
    # nifty 50 index for beta calculation - ^NSEI
    # S&P 500 Index for beta calculation - ^GSPC
...
def query_gpt_4_and_tlm(company_name, stock_data, sentiment_score, risk, strategy, question):
    ...
    prompt_template = PromptTemplate(input_variables=["question"], template=context + "\nQuestion: {question}")
...
def risk_analysis(company_name, question, vector_store):
...
question = "Explain the Technical Analysis and Fundamental Analysis of the Stock and What are the risks associated with investing in Adidas?"

risk_analysis("ADS.DE", question, company_vectors)

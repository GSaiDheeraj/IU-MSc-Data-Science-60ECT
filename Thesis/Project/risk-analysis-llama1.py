...
def process_financial_documents(folder_path):
    ...
    if qdrant_client.collection_exists(collection_name="financial_documents"):
        ...
        for company_name in os.listdir(folder_path):
            ...
            logging.info("Processing documents for {company_name}")
            ...
            if os.path.isdir(company_folder):
                ...
                index = VectorStoreIndex.from_documents(documents=documents, storage_context=storage_context) #vector_store=vector_store)
...
def get_stock_data(company_name, num_months):
    ...
    benchmark = yf.Ticker("^GDAXI")  # S&P 500 Index for beta calculation
    ...
    news_url = f"https://finance.yahoo.com/lookup/?s={company_name}" #f"https://finance.yahoo.com/quote/{company_name}/news?p={company_name}"
...
def query_gpt_4_and_tlm(company_name, stock_data, sentiment_score, risk, strategy, data, question):
    ...
    """
    #Question: {question}
    # qa_tmpl = PromptTemplate(context)
    ...
    gpt_response = openai_mm_llm.complete(messages, image_documents=None) 
...
def risk_analysis(company_name, question, index):
    ...
    gpt_response, trustworthiness_score = query_gpt_4_and_tlm(
        company_name, stock_data, sentiment_score, risk, strategy, context, question
    )
...
def main():
    ...
    response = risk_analysis("ADS.DE", question, company_vectors)

    print(f"Generated Response for ADS.DE: {response}")
